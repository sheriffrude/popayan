 ...
 * @param {any} settings
 */
function OnBeginEditarEmpleado(jqXHR, settings) {
    var data = $(this).serializeObject();
    data["DatosBasicos"] = obtenerDatosBasicos();
    if (data["DatosBasicos"].Foto == null && data["DatosBasicos"].FotoPath!=null && data["DatosBasicos"].FotoPath.length <= 0) {
 ...
 * @param {any} response
 */
function OnCompleteEditarEmpleado(response) {
    var resultado = RequestHttp._ValidateResponse(response);
    if (resultado != null) {
        if (resultado.state == true)
            Utils._BuilderMessage("success", resultado.message, 'RedireccionarVerEmpleado');
        else
            ...
            if (($form.length <= 0) || ($form.length > 0 && $form.valid())) {
                ...
                if ($form.valid()) {
                    ...
                    if ($form.valid()) {
                        ...
                        if ($form.valid()) {
                            // Resumen de registro
                            $("#actualizacion-empleado-form").submit();
                        } else {
...
function RedireccionarVerEmpleado() {
    window.location = URL_VER_EMPLEADO;
}
