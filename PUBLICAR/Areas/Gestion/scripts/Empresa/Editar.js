...
var EmpresaEditar = {
    ...
    OnLoad: function () {
        ...
        EmpresaEditar.EMPRESA_ID = $("#Id").val();
    ...
    AgregarDocumentosLegales: function (empresaId) {
        Utils._BuilderConfirmation("Documentos legales", "¿Desea agregar los documentos legales de la empresa?",
            EmpresaEditar.DocumentosLegales,
            EmpresaEditar.ListarEmpresas, empresaId);
    },
    ...
    UploadImage: {
        Upload: function (e) {
            RequestHttp._UploadFile(e, URL_UPLOAD, function (data) {
                if (data != null) {
                    ...
                    EmpresaEditar.IMAGE.Image = FILE;
        ...
        Crop: function () {
            if (Validations._IsNull(EmpresaEditar.IMAGE.Image)) {
            ...
            EmpresaEditar.IMAGE.DataCrop = $('#logo-view-crop').cropper('getData');
    ...
    OnComplete: function (response) {
        ...
        if (!Validations._IsNull(resultado)) {
            ...
            if (resultado.state)
                Utils._BuilderMessage('success', resultado.message, EmpresaEditar.AgregarDocumentosLegales, empresa);
            else
...
var Empresa_RedSocial = {
    ...
    IMAGE: null,
    OnLoad: function () {
        Empresa_RedSocial.Listar();
    },
    Listar: function () {
        RequestHttp._Post(URL_LISTAR_REDES_SOCIALES_POR_EMPRESA, { empresaId: EmpresaEditar.EMPRESA_ID }, null, function (response) {
            if (!Validations._IsNull(response)) {
                if (response.state) {
                    Empresa_RedSocial.DATA = response.data;
            ...
            "columns": [
                { "data": "NombreRedSocial" },
                {
                    ...
                    "render": function (data, type, full, meta) {
                        return '<img class="" src= "' + data.replace("~", "") + '" />';
                    }
                },
                {
                    "data": "UrlRedSocial",
    ...
    Eliminar: function (id) {
        ...
        for (var i = 0; i < longitudDataRedSocial; i++) {
...
var Empresa_Director = {
    ...
    $TABLA: null,
    OnLoad: function () {
        Empresa_Director.Listar();
    },
    Listar: function () {
        RequestHttp._Post(URL_LISTAR_DIRECTORES_POR_EMPRESA, { empresaId: EmpresaEditar.EMPRESA_ID }, null, function (response) {
            if (!Validations._IsNull(response)) {
                if (response.state) {
                    var data = response.data;
                    for (var i = 0; i < data.length; i++) {
                        Empresa_Director.DATA.push({
                            Id: data[i].Value,
                            Nombre: data[i].Text
                        });
                    }
