...
function initInformacionSalarial() {    
    ...
    if ($("#ObservacionesSalario")!= undefined && $("#ObservacionesSalario").val().length > 0) {
        $("#registra-distribucion-salarial").show();
        $('#DistribucionPago').attr('checked', true);
    }
}
...
function agregarUnidadPorcentaje() {
    ...
    {
        Utils._BuilderMessage('info', 'El procentaje debe ser numérico');
        return false;
    }
    ...
    ActualizarTablaUnidadNegocio($DATA_UNIDADES_PORCENTAJE);
    limpiaDistribucion();
    COUNT_TABLA++;
}
    ...
    $TABLA_UNIDADES_PORCENTAJE = $("#tabla-unidades-porcentaje").dataTable({
        ...
        "ajax": {
            ...
            "dataSrc": function (json) {
                ...
                $DATA_UNIDADES_PORCENTAJE = json.data;
                return json.data;
            }
        },
        "columns": [
 ...
 * Evento que crea por jquery la tabla temporal de hijos
 */
function ActualizarTablaUnidadNegocio(dataRows) {
    $TABLA_UNIDADES_PORCENTAJE = $("#tabla-unidades-porcentaje").dataTable({
        "bFilter": false,
        "bLengthChange": false,
        "destroy": true,
